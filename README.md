...
### Идея эксперимента:
Возьмём Contextual рекомендер и сделаем так, чтобы он предлагал трек, похожий на последний хороший трек,
если предыдущий был плохим (плохой - время его прослушивания меньше border_of_good_track).

### Детали:
Сравним время прослушивания прошлого трека с border_of_good_track:
- если время меньше, то значит, что трек плохой,
поэтому заменяем предыдущий трек на последний хороший и определяем следующую рекомендацию уже относительно него.
- если время больше, то значит, что трек хороший, поэтому ничего не заменяем,
но обновляем последний хороший трек текущим.

### Результат эксперимента:
![Результат А/В эксперимента](result.png)

## Инструкция

Запускается всё, как на практиках. Нужно только в файле sim/sim/run.py поменять на 15 строчке путь на свой.
Jupiter notebook с анализом эксперимента называется Homework.ipynb и лежит в папке jupiter.
